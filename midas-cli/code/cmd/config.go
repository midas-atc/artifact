...
func NewConfigCommand(cli *midascli.midasCli) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Configure user's account in midas CLI",
		Run: func(cmd *cobra.Command, args []string) {
			...
			var config midascli.UserConfig
	...
	var userName string
	var authFile string
	cmd.Flags().StringVarP(&userName, "username", "u", cli.UserConfig("username")[0], "Configure MIDAS username")
	cmd.Flags().StringVarP(&authFile, "file", "f", cli.UserConfig("authfile")[0], "Configure MIDAS authentication file path")
